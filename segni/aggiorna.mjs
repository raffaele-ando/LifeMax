/* RIFÀ IL PACCO da una copia di lucide-static.
   ...
   Riscrive lucide.json (nome → frammento SVG) e parole.json (nome → parole
   per cercarlo). Non tocca src/segni/segni.ts: i segni già presi restano quelli
...
fs.writeFileSync(path.join(QUI, 'lucide.json'), JSON.stringify(fuori));
...
const ico = fs.readFileSync(path.join(QUI, '..', 'src', 'segni', 'segni.ts'), 'utf8');
const presi = [...ico.matchAll(/\/\* *lucide:([a-z0-9-]+) *—/g)].map((m) => m[1]);
const spariti = presi.filter((n) => !fuori[n]);
console.log('  ' + Object.keys(fuori).length + ' segni nella libreria, ' + presi.length + ' già presi nell’app');
