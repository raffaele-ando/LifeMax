/* ============================================================
   ...
   Nel sito di prima erano `window.ICO`, `window.LOGO`, `window.GOOGLE_G`,
   messi lì da uno `<script>`. Qui sono tre export di un modulo, e restano
   anche su `window` finché il vecchio `app.js` gira accanto a questo.
   ============================================================ */
import { presa } from '../tipi/presa';

/* `Record<string, string>` e non un'unione dei nomi possibili: i nomi si
   scelgono a runtime — l'icona di un'area la sceglie chi usa l'app, e arriva
   dai dati come stringa qualunque. Chiedere qui un nome noto vorrebbe dire un
   controllo in ogni punto che disegna un'area. Il segno che non c'è ha già la
   sua risposta dichiarata due dita più sotto: il pallino. */
...
const SENSO: Record<string, string> = {
  /* le pagine */
...
const GENERICI: string[] = ['check', 'plus', 'x', 'trash', 'arrowRight', 'chevronGiu', 'save'];
...
const SCALA: number[] = [11, 13, 15, 18, 26];
function gradino(n?: number): number {
  /* `n` arriva anche vuoto: il caso normale è chiamare `ICO('check')` senza
     misura, e il gradino della riga è il ripiego. */
  const m = Number(n) || 15;
  let vicino = presa(SCALA[0]);
  for (let i = 1; i < SCALA.length; i++) {
    ...
    const g = presa(SCALA[i]);
    if (Math.abs(g - m) <= Math.abs(vicino - m)) vicino = g;
  }
  /* oltre la scala (loghi, illustrazioni) si passa la misura così com'è */
  return m > 30 ? m : vicino;
}
function tratto(size: number): number {
  const sw = 30 / size;
  return Math.round(Math.min(2.5, Math.max(1.15, sw)) * 100) / 100;
}
/* IL SEGNO, COME STRINGA DI SVG. Torna testo e non un elemento, ed è la
   stessa scelta di prima: nel sito vecchio serviva perché tutto si costruiva
   con `innerHTML`, e serve ancora perché `dangerouslySetInnerHTML` è l'unica
   strada per mettere in una pagina React uno SVG che arriva da una tabella.
   Il pezzo React che lo fa si chiama `Disegno`, ed è uno solo — così il
   punto in cui questa stringa entra nel DOM si conta sulle dita di zero
   mani. */
function segno(nome: string, size?: number, cls?: string): string {
  const d = PATHS[nome];
  if (!d) return '<span class="ico-pallino"></span>';
  const s = gradino(size);
  return '<svg class="ico' + (cls ? ' ' + cls : '') + '" width="' + s + '" height="' + s +
    '" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="' + tratto(s) +
...
function GOOGLE_G(size?: number): string {
  /* anche il marchio Google passa dalla scala: se resta fuori si siede a
     mezzo pixel di distanza dal testo che gli sta accanto */
  const s = gradino(size || 15);
  return '<svg width="' + s + '" height="' + s + '" viewBox="0 0 48 48" aria-hidden="true" style="flex:none;vertical-align:-3px">' +
...
/* i quattro campi attaccati alla funzione, come prima: `prove/segni.js` li
   legge da qui, e `SENSO` è la tabella dei significati che quella prova
   controlla riga per riga. */
export const ICO = Object.assign(segno, {
  SCALA: SCALA,
  SENSO: SENSO,
  GENERICI: GENERICI,
  NOMI: Object.keys(PATHS)
});
