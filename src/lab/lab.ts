/* ============================================================
   ...
   COME ARRIVA A CHI LO APRE
   Nel sito di prima era `window.LM_LAB`, e la pagina lo chiamava con un
   contenitore in mano. Qui è un modulo: `montaIn(el)` è la stessa cosa, e in
   React ci si arriva da un `useEffect` con un `ref` — quel telaio scrive
...
type Schermo = 'oggi' | 'attivita';

/* UN MOCK. `n` non si scrive a mano: lo mette `DESIGN.forEach` dalla
...
/* IL CONTENUTO, DICHIARATO. Dieci mock lo leggono, e per tutta la vita del
   file era un oggetto senza forma: `p.badge` esiste per una pagina su cinque,
   `b.adesso` per un blocco su quattro, e chi ne aggiungeva uno nuovo se ne
   accorgeva guardando l'altro mock accanto. */
/* UNA COSA DELLA GIORNATA. Quella con un orario e quella senza sono la
   stessa riga: sette mock su dieci le mettono in fila con `concat`, e la
   differenza fra le due è esattamente quello che disegnano («senza ora»).
   Scritte come due forme distinte, `concat` non compilava — e in JavaScript
   passava, restituendo un elenco misto su cui si leggeva `b.fatto` anche a
   chi non ce l'ha. */
interface Voce { t: string; area: string; da?: string; a?: string; fatto?: boolean; adesso?: boolean }
interface AbitudineDemo { t: string; fatta: boolean; serie: number }
interface DaFare { t: string; area: string; quando: string; urgente?: boolean }
interface Pagina { id: string; nome: string; ico: string; badge?: number }
...
function pc(hhmm: string): number {
  const p = hhmm.split(':'), m = +(p[0] || 0) * 60 + +(p[1] || 0);
  const a = 7 * 60 + 30, b = 23 * 60 + 30;
  return Math.max(0, Math.min(100, (m - a) / (b - a) * 100));
}
function perc(): number { return Math.round(C.fatte / C.totali * 100); }
...
const D2: Mock = {
  ...
  attivita: function () {
    const riga = function (b: DaFare, primo: boolean) {
...
const D9: Mock = {
  ...
  oggi: function () {
    ...
    const px = function (hhmm: string) { return Math.round(pc(hhmm) * H / 100); };
    return '<div class="a9">' +
      ...
      '<div class="a9-tela">' +
        ...
        '<div class="a9-pista" style="height:' + H + 'px">' +
          ...
          C.blocchi.map(function (b) {
            /* i blocchi hanno tutti un'ora: la tabella sta duecento righe
               sopra, in questo stesso file */
            const t = px(presa(b.da)), h = px(presa(b.a)) - t, corto = h < 48;
    ...
    return '<div class="p10">' +
      '<div class="p10-scorri">' +
        ...
        '<section class="p10-carta">' +
          ...
          '<h2 class="lab-h">' + esc(presa(C.prima[0]).t) + '</h2>' +
          '<p class="p10-quando p10-rosso">' + presa(C.prima[0]).quando + ' · ' + presa(C.prima[0]).area + '</p>' +
...
const DESIGN: Mock[] = [D1, D2, D3, D4, D5, D6, D7, D8, D9, D10];
/* il numero lo prende dalla posizione: scriverlo a mano dieci volte era il
   modo di averne due col cinque dopo uno spostamento */
...
function leggi(k: string, dif: string): string {
  try { const v = localStorage.getItem(CHIAVE + '.' + k); return v == null ? dif : v; } catch { return dif; }
}
function scrivi(k: string, v: string): void {
  try { localStorage.setItem(CHIAVE + '.' + k, v); } catch { /* ignora */ }
}

function montaIn(el: HTMLElement): void {
  let scelto = Math.min(10, Math.max(1, +leggi('n', '1') || 1));
  let schermo: Schermo = leggi('schermo', 'oggi') === 'attivita' ? 'attivita' : 'oggi';
  let largo = leggi('largo', 'si') !== 'no';
  ...
  /* i quattro nodi si prendono una volta: sono figli di quello che abbiamo
     appena scritto noi, quindi ci sono — e `presa` dice perché */
  const demo = presa(el.querySelector<HTMLElement>('#lab-demo'));
  const corpo = presa(el.querySelector<HTMLElement>('#lab-schermo-corpo'));
  const nome = presa(el.querySelector<HTMLElement>('#lab-nome'));
  const stanza = presa(el.querySelector<HTMLElement>('#lab-stanza'));

  function disegna(muoviFuoco?: boolean): void {
    const d = presa(DESIGN[scelto - 1]);
    ...
    el.querySelectorAll<HTMLElement>('.lab-tab').forEach(function (t) {
      const on = +(t.getAttribute('data-d') || 0) === scelto;
      t.classList.toggle('on', on);
      t.setAttribute('aria-selected', String(on));
      if (on) { t.scrollIntoView({ block: 'nearest', inline: 'nearest' }); if (muoviFuoco) t.focus({ preventScroll: true }); }
    });
    el.querySelectorAll<HTMLElement>('#lab-schermo [data-s]').forEach(function (b) { b.classList.toggle('on', b.getAttribute('data-s') === schermo); });
    el.querySelectorAll<HTMLElement>('#lab-largo [data-w]').forEach(function (b) { b.classList.toggle('on', (b.getAttribute('data-w') === 'si') === largo); });
    scrivi('n', String(scelto)); scrivi('schermo', schermo); scrivi('largo', largo ? 'si' : 'no');
  }

  el.querySelectorAll<HTMLElement>('.lab-tab').forEach(function (t) {
    t.addEventListener('click', function () { scelto = +(t.getAttribute('data-d') || 1); disegna(); });
  });
  el.querySelectorAll<HTMLElement>('#lab-schermo [data-s]').forEach(function (b) {
    b.addEventListener('click', function () { schermo = b.getAttribute('data-s') === 'attivita' ? 'attivita' : 'oggi'; disegna(); });
  });
  el.querySelectorAll<HTMLElement>('#lab-largo [data-w]').forEach(function (b) {
    b.addEventListener('click', function () { largo = b.getAttribute('data-w') === 'si'; disegna(); });
  });
  presa(el.querySelector<HTMLElement>('.lab-rail')).addEventListener('keydown', function (e) {
