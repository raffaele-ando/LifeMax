...
function RigaSc({ eti, cls, children }) {
  return <div className={'lista-riga sc-riga' + (cls ? ' ' + cls : '')}>
    <span className="sc-eti">{eti}</span>{children}
  </div>;
}

export default function Lezione({ id }) {
  const a = A();
  const [, bump] = useState(0);
  const l = a.trovaLezione(id);
  const testo = useRef(null);
  const area = useRef(null);
  usaCambioNativo(area, (i) => {
    window.LM.modificaLezione(id, { areaId: i.value || null });
    a.ridisegnaLezioni();
  }, [id]);

  if (!l) { a.chiudiSheet(); a.ridisegnaAtt(); return null; }
  const esp = l.espId ? window.LM.load().esperimenti.find((x) => x.id === l.espId) : null;
  const ridisegna = () => { bump((n) => n + 1); a.ridisegnaLezioni(); };
  ...
  return (
    <div className="sc">
      ...
      <textarea key={l.testo} id="lez-testo" rows="2" ref={testo} defaultValue={l.testo}
        onBlur={() => {
          const v = testo.current.value.trim();
          const ora = a.trovaLezione(id);
          if (v && ora && v !== ora.testo) { window.LM.modificaLezione(id, { testo: v }); a.ridisegnaLezioni(); }
        }} />

      <div className="lista-eti"><Segno nome="confronto" dim={11} />Evidenza</div>
      <div className="q-chips">
        {window.LM.FORZE_LEZIONE.map((f) => (
          <button key={f.id} className={'q-chip' + (f.id === l.forza ? ' on' : '')} data-forza={f.id}
            onClick={() => { window.LM.modificaLezione(id, { forza: f.id }); ridisegna(); }}>{f.eti}</button>
        ))}
      </div>
      ...
      <div className="lista">
        <RigaSc eti="Esito" cls="sc-riga-alta">
          <span className="sc-val q-chips">
            <button className={'q-chip' + (l.verso === 'si' ? ' on' : '')} data-verso="si"
              onClick={() => { window.LM.modificaLezione(id, { verso: 'si' }); ridisegna(); }}>
              <Segno nome="funziona" dim={13} /> funziona</button>
            <button className={'q-chip' + (l.verso === 'no' ? ' on' : '')} data-verso="no"
              onClick={() => { window.LM.modificaLezione(id, { verso: 'no' }); ridisegna(); }}>
              <Segno nome="nonFunziona" dim={13} /> non funziona</button>
          </span>
        </RigaSc>
        <RigaSc eti="Area">
          <span className="sc-val" ref={area} {...html(a.selectAreeOpz('lez-area', l.areaId, 'sc-inline'))} />
        </RigaSc>
        {esp ? <RigaSc eti="Misurato con"><span className="sc-val">{esp.nome}</span></RigaSc> : null}
        <RigaSc eti="Scritta"><span className="sc-val">{window.LM.fmtShort(window.LM.dayKey(new Date(l.creata)))}</span></RigaSc>
      </div>
      ...
      <div className="lista mt">
        ...
        <button className="lista-riga sc-riga sc-tocca" id="lez-esp" onClick={() => {
          a.lezDaProvare = a.trovaLezione(id);
          a.chiudiSheet();
        ...
        <button className="lista-riga sc-riga sc-tocca sc-pericolo" id="lez-del" onClick={() => {
          a.conAnnulla('Riga tolta.', 'trash', () => { window.LM.rimuoviLezione(id); });
          a.chiudiSheet(); a.ridisegnaLezioni();
        }}>
