...
const PANNELLI=[
  ...
  ['Impostazioni → Come ti avviso', async()=>{await vaiA('oggi');await p.evaluate(()=>{const b=[...document.querySelectorAll('button')].find(x=>/impostazioni/i.test(x.getAttribute('aria-label')||x.title||x.textContent));b.click();});await p.waitForTimeout(600);await p.evaluate(()=>{const b=[...document.querySelectorAll('.sheet button')].find(x=>/Promemoria/i.test(x.textContent));if(b)b.click();});}],
